 ...
 * 计算验证码
 */
class ComputeCaptcha implements Captcha
{
    /**
     * @var Redis
     */
     ...
     * @var string 缓存前缀
     */
    private $cache_prefix = 'clown:captcha:compute:';
...
//        '÷'
    ...
    public function captcha($key = '')
    {
        ...
        //获取计算方式, 根据计算方式得到数字和结果
        $content = $this->getCompute($this->getComp());
        //将结果写入缓存
        $this->redis->set($this->cache_prefix . $key, $content['result'], $this->expire);
        $content['result'] = '=';
    ...
    /**
     * 根据计算方式生成数字和结果
     * @param $comp string 计算方式
     * @return array
     */
    public function getCompute($comp = '')
    {
        switch ($comp){
            case '+':
                $num1 = mt_rand(1, 10);
        ...
        return ['num1' => $num1, 'comp' => $comp, 'num2' => $num2, 'result' => $result];
    }
    ...
    {
        $comp = $this->compute[mt_rand(0, count($this->compute) - 1)];
